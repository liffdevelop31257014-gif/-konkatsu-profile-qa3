...
/* ------------------------------------------------------------
   Q9 詳細欄の表示/非表示
   a9-4（毎回プレゼント）/ a9-5（毎回ではないがプレゼント）を
   選択した場合のみ詳細欄を表示する
   ------------------------------------------------------------ */
function toggleQ9Detail(value) {
  const el = document.getElementById("q9Detail");
  if (!el) return;
  const show = (value === "a9-4" || value === "a9-5");
...
function restoreFormData(data) {
  ...
  if (data.q9) {
    const r = document.querySelector(`input[name="q9"][value="${data.q9}"]`);
    if (r) r.checked = true;
...
function validate(data) {
  ...
  if (!data.q9)                 errors.push("Q9: イベントごとの過ごし方を選択してください。");
  if ((data.q9 === "a9-4" || data.q9 === "a9-5") && !data.q9Detail.trim())
...
/* ------------------------------------------------------------
   ...
   Q4/Q6はテキストではなくインデックス番号で持たせてURLを短縮する
   ------------------------------------------------------------ */
...
function renderViewMode(data, options = {}) {
  ...
  const q9Answer = (() => {
    const base = q9Labels[data.q9] || "未回答";
    if ((data.q9 === "a9-4" || data.q9 === "a9-5") && data.q9Detail) {
      return `${base}\n（詳細：${data.q9Detail}）`;
    }
    return base;
  })();
  ...
  const rows = [
    ...
    { q: "Q9 記念日や誕生日、クリスマスなどのイベントごとはどう過ごしたいですか？", a: q9Answer },
...
(async () => {
  ...
  /* ----- Q9 ラジオ：詳細欄の表示制御 ----- */
  document.querySelectorAll('input[name="q9"]').forEach(r =>
    r.addEventListener("change", () => toggleQ9Detail(r.value))
  );
  const q9Checked = document.querySelector('input[name="q9"]:checked');
  toggleQ9Detail(q9Checked ? q9Checked.value : "");
  ...
  document.getElementById("clearBtn").addEventListener("click", () => {
    ...
    ["q1good","q1bad","q2good","q2bad","q3","q7","q8","q9Detail"].forEach(id => {
      const el = document.getElementById(id);
      if (el) el.value = "";
    });
  ...
  document.getElementById("shareBtn").addEventListener("click", async () => {
    ...
    // 送信＆共有完了 → 本人にも共有URLを送信し、続けてPart4への案内も送信
